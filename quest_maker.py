...
moderator = OpenAI(http_client=http_client)
...
def generate_full_story_description(state:ConceptFields):
  ...
  return chain.invoke({'pretty_themes':pretty_themes, 'pretty_npcs':pretty_npcs, 'description':state.description, 'additional_requirements':state.additional_requirements})
...
def generate_story_points(state:ConceptFields, full_description:str):
  ...
  return chain.invoke({'pretty_themes':pretty_themes, 'format_instructions':story_points_instructions, 'full_description':full_description})
...
def make_task(story_point:dict, state:ConceptFields, i:int):
  ...
  return chain.invoke({'theme':state.themes_list[i], 'format_instructions':task_instructions})
...
def generate_replika(story_point:dict, state:ConceptFields, i:int):
  ...
  return chain.invoke({'description':description, 'theme':theme, 'character_description':npc_dict['character_description']})

async def get_skazka(state:ConceptFields):
  #somehow get state
...
async def save_to_db(skazka:dict):
